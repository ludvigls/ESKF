...
def quaternion_product(ql: np.ndarray, qr: np.ndarray) -> np.ndarray:
    ...
    quaternion = np.matmul(eta_left*np.eye((6,6))+np.array([[np.zeros((3,3)),-epsilon_left.T],[eta_left,-cross_product_matrix(epsilon_left)]),q_right)# TODO: Implement quaternion product
...
def quaternion_to_rotation_matrix(
    ...
    R = np.eye(3)+2*eta*cross_eps+2*np.matmul(cross_eps*cross_eps) # TODO: Convert from quaternion to rotation matrix
...
def quaternion_to_euler(quaternion: np.ndarray) -> np.ndarray:
    ...
    phi = np.arctan2(2*quaternion[3]*quaternion[2]+quaternion[0]*quaternion[1],quaternion_squared-quaternion[1]**2-quaternion[2]**2+quaternion[3]**2)  # TODO: Convert from quaternion to euler angles
    theta = np.arcsin(2*(quaternion[0]*quaternion[2]-quaternion[1]*quaternion[3]))  # TODO: Convert from quaternion to euler angles
    psi =  np.arctan2(2*quaternion[1]*quaternion[2]+quaternion[0]*quaternion[3],quaternion_squared+quaternion[1]**2-quaternion[2]**2-quaternion[3]**2)
